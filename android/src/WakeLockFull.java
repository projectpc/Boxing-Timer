...
public class WakeLockFull
{
    private Activity myActivity;
    protected PowerManager.WakeLock m_WakeLock = null;

    public WakeLockFull(Activity a)
    {
        myActivity = a;
    }
    ...
    {
        System.out.println("Inside WakeLockFull::configure");

         try
         {
             final PowerManager pm = (PowerManager) myActivity.getSystemService(Context.POWER_SERVICE);
              //FULL_WAKE_LOCK - полная яркость  SCREEN_DIM_WAKE_LOCK - подзатемнить
            m_WakeLock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK , "My Tag");
         ...
         {
             System.out.println("WakeLockFull failed: " + e.toString());
         }
